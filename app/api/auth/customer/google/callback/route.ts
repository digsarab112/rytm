...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || !storedState || state !== storedState) {
    return NextResponse.redirect(
      new URL(`/auth/login?locale=${locale}&google=invalid-state`, origin),
    );
  }
  ...
  try {
    ...
    const response = NextResponse.redirect(
      new URL(`/auth/google/complete?locale=${locale}`, origin),
    );
    ...
    return NextResponse.redirect(
      new URL(`/auth/login?locale=${locale}&google=failed`, origin),
    );
  }
}
